...
def more_boar(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    player_left = get_boar(player_score)
    opponent_left = get_boar(opponent_score)
    return player_left[0] < opponent_left[0] and player_left[1] < opponent_left[1]
    # END PROBLEM 4

def get_boar(score):
    """
     辅助函数,获取一个数字最左边两位,如果数字是单数,补0
     >>> get_boar(123)
     12
     >>> get_boar(4)
     04
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max_averaged, max_rolls = 0, 1
    for i in range(1, 11):
        averaged = make_averaged(roll_dice, trials_count)(i, dice)
        if max_averaged < averaged:
            max_averaged = averaged
            max_rolls = i
    return max_rolls
    # END PROBLEM 9
